...
// The traversal itself. The state it threads down and the results it collects live
// in `HierarchyWalk.swift`; what is here is the part that genuinely extends
// ``SensitiveViewManager``, because it reads the manager's sensitivity classification,
// registered classes and caches.
//
...
// than by review — every write goes through ``HierarchyWalk`` (its own file),
