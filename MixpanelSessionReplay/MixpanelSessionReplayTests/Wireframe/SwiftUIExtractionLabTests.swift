...
final class SwiftUIExtractionLabTests: XCTestCase {
    ...
    func test_runAllStrategies_dumpFindings() throws {
        ...
        for (i, sub) in host.view.subviews.enumerated() {
            lines.append(
                "subview[\(i)] \(type(of: sub)) a11yElements=\(String(describing: sub.accessibilityElements)) isA11yElement=\(sub.isAccessibilityElement) label=\(String(describing: sub.accessibilityLabel))"
            )
        }
        lines.append("")
...
private func dumpTopology(_ v: UIView, depth: Int, out: inout [String]) {
    ...
    out.append(
        "\(indent)V \(cls) frame=\(v.frame) a11yLabel=\(v.accessibilityLabel ?? "-") a11yValue=\(v.accessibilityValue ?? "-")"
    )
