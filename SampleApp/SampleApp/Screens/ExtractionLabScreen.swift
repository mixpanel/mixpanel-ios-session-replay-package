...
private enum Strategy: String, CaseIterable, Identifiable {
  ...
  var detail: String {
    switch self {
    case .accessibilityWalk:
      return "Walks accessibilityElements / accessibilityLabel / accessibilityValue on every subview and CALayer. Public UIKit."
    case .rootViewMirror:
      return "Mirror-recurses into _UIHostingView.mirror(\"_rootView\") — where SwiftUI's value tree lives — collecting String / LocalizedStringKey / verbatim leaves."
    case .leafIvarMirror:
      return "Deep Mirror + ObjC ivar walk on CGDrawingView (iOS ≤18) and CGDrawingLayer (iOS 26+) — the current SDK strategy, plus recursion into value-typed fields."
    case .allLayerMirror:
      return "Mirror every UIView + every CALayer under the hosting view, up to depth 4, collecting String/AttributedString leaves."
    }
  }
