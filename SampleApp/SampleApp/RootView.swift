        ...
        VStack(spacing: 4) {
          if console.emitCount == 0 {
            ...
            Text("No elements with text on this screen.")
              .bold()
            Text("\(console.latestElementCount) elements in latest frame — all have text=nil. Toggle the filter to see them.")
        ...
        ScrollView {
          LazyVStack(alignment: .leading, spacing: 2, pinnedViews: []) {
            ...
            if console.latestElementCount > console.rows.count {
              Text("… \(console.latestElementCount - console.rows.count) more (raise WireframeConsole.maxDisplayedRows to see)")
                .font(.caption2)
                .foregroundColor(.secondary)
                .padding(8)
            }
